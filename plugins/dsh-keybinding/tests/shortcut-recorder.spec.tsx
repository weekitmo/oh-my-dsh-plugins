...
describe('ShortcutRecorder', () => {
  it('shows recording state, normalizes a keydown, and writes it back', () => {
    ...
    expect((input as HTMLInputElement).value).toBe('Listening for shortcut...')
  ...
  it('builds a shortcut with the modifier buttons', () => {
    ...
    const input = screen.getByRole('textbox', { name: 'Workspace hints' })
    fireEvent.click(input)
    fireEvent.click(screen.getByRole('button', { name: 'Shift modifier' }))
    expect(screen.getByRole('button', { name: 'Shift modifier' }).getAttribute('aria-pressed')).toBe('true')
