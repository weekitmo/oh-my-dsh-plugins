...
- `2026-09-17-delegate-trace-rpc-broken/` - fix the delegate-agent and trace plugins on DSH 0.1.5-rc.2: their `connection.rpc.handle` channels never registered (registration resolves `webServer` through the Connection plugin's own fiber chain), so both plugins now use authenticated `POST /api/<namespace>/<method>` routes; released as `v0.1.5`
- `2026-09-18-dsh-notify-timer-fix-and-sounds/` - root-cause the missing browser notifications (the client convergence timer called a bare `setTimeout` as a method, so browsers threw `TypeError: Illegal invocation` while Node tests stayed green), then add waiting-for-approval alerts and per-outcome result sounds inlined into the client bundle
